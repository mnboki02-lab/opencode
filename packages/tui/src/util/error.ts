...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `1hit models` to list available models",
      "Or check your config (1hit.json) provider/model names",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, 1hit does not support MCP authentication yet.`
  }
  return undefined
}
