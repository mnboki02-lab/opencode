...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("1hit.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at 1hit",
  ...
  "Create {highlight}1hit.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/1hit/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.1hit/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.1hit/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.1hit/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.1hit/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent 1hit from reading sensitive files",
  "Use {highlight}1hit run{/highlight} for non-interactive scripting",
  "Use {highlight}1hit --continue{/highlight} to resume the last session",
  "Use {highlight}1hit run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}1hit serve{/highlight} for headless API access to 1HIT",
  "Use {highlight}1hit run --attach{/highlight} to connect to a running server",
  "Run {highlight}1hit upgrade{/highlight} to update to the latest version",
  "Run {highlight}1hit auth list{/highlight} to see all configured providers",
  "Run {highlight}1hit agent create{/highlight} for guided agent creation",
  ...
  "Create JSON theme files in {highlight}.1hit/themes/{/highlight} directory",
  ...
  "Run {highlight}1hit debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/mnboki02-lab/opencode{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with 1HIT for curated, tested models",
